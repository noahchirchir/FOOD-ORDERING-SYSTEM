...
def list_menu():
    ...
    for item in menu_items:
        print(
            f"ID: {item.id}, Name: {item.name}, Item: {item.item_name}, Price: {item.price}"
        )
...
def list_orders():
    ...
    for order in orders:
        print(
            f"ID: {order.id}, Customer ID: {order.customer_id}, Order Date: {order.order_date}"
        )
...
def update_order():
    ...
    order_date = input(
        "Enter new order date (YYYY-MM-DD HH:MM:SS) (leave blank to keep current): "
    )
    order = Order.update(
        order_id,
        customer_id=int(customer_id) if customer_id else None,
        order_date=(
            datetime.strptime(order_date, "%Y-%m-%d %H:%M:%S") if order_date else None
        ),
    )
    if order:
...
def list_reviews():
    ...
    for review in reviews:
        print(
            f"ID: {review.id}, Customer ID: {review.customer_id}, Menu ID: {review.menu_id}, Rating: {review.rating}, Comment: {review.comment}"
        )
...
def create_review():
    ...
    review = Review.create(
        customer_id=customer_id, menu_id=menu_id, rating=rating, comment=comment
    )
...
def update_review():
    ...
    review = Review.update(
        review_id, rating=int(rating) if rating else None, comment=comment or None
    )
    if review:
...
def list_order_items():
    ...
    for item in order_items:
        print(
            f"ID: {item.id}, Order ID: {item.order_id}, Menu ID: {item.menu_id}, Quantity: {item.quantity}"
        )
